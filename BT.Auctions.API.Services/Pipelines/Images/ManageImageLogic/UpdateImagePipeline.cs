...
namespace BT.Auctions.API.Services.Pipelines.Images.ManageImageLogic
{
    ...
    public class UpdateImagePipeline : LogicPipeline<Image>
    {
        public UpdateImagePipeline(ILogger<Image> logger, IImageRepository imageRepository, ILotRepository lotRepository) : base(logger)
        {
            Add(new ValidateImageHasUrl());
            Add(new ValidateImageLotExists(lotRepository));
            Add(new UpdateImage(imageRepository));
        }
    }
}